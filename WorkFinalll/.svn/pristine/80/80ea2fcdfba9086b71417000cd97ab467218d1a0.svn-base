        ...
        layui.use('layer', function() {
            ...
            $('#select-btn').click(function () {
                ...
                if(!isNaN(input1)&&input1!="") {
                    ...
                    setTimeout(function () {
                        $.get("${pageContext.request.contextPath}/privilegeManage/powerloadById.action",{"power.powerId":input1},function(data){
                            var table = $("#selectTable") ;
                            var tr = "<tr>" ;
                            var td = "<td>" + data.powerId + "</td>" +
                                    "<td>" + data.powerName + "</td>" +
                                    "<td>" + data.powerType + "</td>"+
                                    "<td>" + data.powerUrl + "</td>"+
                                    "<td>" + data.powerCode + "</td>";
                            tr = tr + td + "</tr>" ;
                            $(table).append(tr) ;
                        });
                        layer.close(index);
...
<div class="admin-main">
    <div id="main">
        <span class="title1">查询权限</span>
        <div class="main-center">
                <div class="layui-form-item">
                    <label class="layui-form-label">权限编号</label>
                    <div class="layui-input-block">
                        <input type="text" name="power.powerId" required  lay-verify="required" placeholder="请输入权限编号" autocomplete="off" class="layui-input" id="layui-input1">
                    </div>
